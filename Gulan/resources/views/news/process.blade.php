...
<header>
  ...
  @include('sweetalert::alert')
    ...
    <div class="container mt-5 bg-blue ">
        ...
        <h1 class="text-center text-3xl font-bold" >Add your posts</h1><br>
        <hr><br>
       <form action="{{url('user_post')}}" method="POST" enctype="multipart/form-data" class="text-start ">
             @csrf

                <select name="category_id" style="border:1px solid white; " class="block appearance-none w-full bg-white border border-gray-400 hover:border-gray-500 px-4 py-2 pr-8 rounded shadow leading-tight focus:outline-none focus:shadow-outline ">
                   @foreach ($categories as $category)
                       <option value="{{ $category->id }}">{{ $category->name }} </option>
                   @endforeach
                </select><br>
                    <input name="title" type="text " placeholder="post title" class="  block appearance-none w-full bg-white border border-gray-400 hover:border-gray-500 px-4 py-2 pr-8 rounded shadow leading-tight focus:outline-none focus:shadow-outline  text-black my-3 text-center">
                    <textarea name="description" class=" form-control block appearance-none w-full bg-white border border-gray-400 hover:border-gray-500 px-4 py-2 pr-8 rounded shadow leading-tight focus:outline-none focus:shadow-outline text-black my-3  text-center" placeholder="discription"></textarea>
                    <input style="background-image: linear-gradient(-20deg, #616161 0%, #9bc5c3 100%);" class="   text-white font-semibold hover:text-white py-2 px-4 border border-blue-500 hover:border-transparent rounded " type="submit" value="Add Post">
